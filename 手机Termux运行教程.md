...
随着项目的迭代，可能会有新功能发布。在 Termux 中更新代码非常简单：

1.  **运行更新脚本**：
    在项目目录下运行以下命令即可自动清理冲突并拉取最新代码：
    ```bash
    bash update.sh
    ```
    *   如果提示 `permission denied`，可以先运行 `chmod +x update.sh`。
    *   脚本会自动处理 `git pull` 时的常见冲突（如 `dev-dist/sw.js` 错误）。
