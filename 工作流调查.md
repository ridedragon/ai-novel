...
### 5.3 移动端同步修复与性能对齐 (针对 4.5) - [已完成]

- **现象**：发现手机端在执行工作流时仍存在类似卡顿和主进程压力。
- **修复方案**：
  - **渲染层级优化**：对 [`src/components/MobileWorkflowEditor.tsx`](src/components/MobileWorkflowEditor.tsx) 应用了与 PC 端一致的 SVG 合并策略，移除了内联样式节点。
  - **增量上报同步**：将移动端工作流的 `onNovelUpdate` 回调修改为支持增量 Map 合并模式，减小手机端 IPC 通道负荷。
  - **异步分片解析**：针对移动端更弱的 CPU 性能，对 AI 输出的大型 JSON 结构化解析引入了异步 yield 机制，防止解析阶段导致 UI 彻底锁死。
