...
### 问题定位
经过详细分析，发现问题出在 `useWorkflowEngine.ts` 文件中创建 `AutoWriteEngine` 实例时，没有明确设置 `stream` 属性。虽然代码通过 `...globalConfig` 展开了全局配置，但在某些情况下，全局配置中的 `stream` 属性可能未被正确设置或被覆盖，导致流式输出被禁用。

### 修复方案
在 `useWorkflowEngine.ts` 文件中，修改 `engCfg` 对象的创建，明确添加 `stream: true` 属性，确保流式输出始终启用：
...
### 修复原理
1. **明确启用流式输出**：通过在配置对象中明确设置 `stream: true`，确保无论全局配置如何，流式输出都会被启用
2. **保持兼容性**：通过将 `stream: true` 放在 `...globalConfig` 之前，确保全局配置中的 `stream` 属性可以覆盖默认值（如果需要）
3. **最小化修改**：只修改了必要的代码，保持了其他逻辑的完整性

### 验证步骤
1. 启动开发服务器：`npm run dev`
2. 打开浏览器访问 `http://localhost:8002`
3. 尝试生成正文内容
4. 观察浏览器控制台，确认是否有 `[AutoWrite Stream]` 开头的日志
5. 观察界面，确认内容是否以流式方式显示，而不是一次性显示

### 预期效果
修复后，正文界面应该能够正常显示流式输出效果，AI 生成内容时会逐字显示，而不是等全部生成完成后一次性显示。