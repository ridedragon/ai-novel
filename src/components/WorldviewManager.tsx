...
interface WorldviewManagerProps {
  ...
  // AI Generation Props
  onGenerateWorldview?: (mode?: 'generate' | 'chat') => void
...
export const WorldviewManager: React.FC<WorldviewManagerProps> = ({
  ...
  const handleClearAll = () => {
    ...
    setConfirmState({
      isOpen: true,
      title: '清空世界观',
      message: '确定要清空当前世界观的所有设定吗？此操作无法撤销。',
      onConfirm: () => {
        updateEntries([])
        setConfirmState(prev => ({ ...prev, isOpen: false }))
      }
               ...
               <div className="p-3 md:p-4 border-b border-gray-700 bg-gray-800/50 flex flex-wrap items-center justify-between gap-3 md:gap-4 shrink-0 z-10 backdrop-blur-sm sticky top-0">
                  ...
                  <div className="flex items-center gap-1 md:gap-2">
                     ...
                     <button 
                        ...
                        disabled={activeSet.entries.length === 0}
                     ...
                     <div className="flex-1 flex flex-col w-full h-full">
                        <div className="flex items-center justify-between mb-4 shrink-0">
                           <div className="flex items-center gap-2 text-gray-400">
                              ...
                              <span className="text-sm font-medium">世界观讨论对话</span>
                           </div>
                                 ...
                                 <textarea
                                    ...
                                    onKeyDown={(e) => {
                                       if (e.key === 'Enter' && !e.shiftKey && !isGenerating) {
                                          e.preventDefault();
                                          onGenerateWorldview && onGenerateWorldview('chat');
                                       }
                                    }}
                                          ...
                                          onClick={() => {
                                             if (onReturnToMainWithContent) {
                                                ...
                                                const formattedHistory = history.map(m =>
                                                   `${m.role === 'user' ? '用户' : '世界观助手'}: ${m.content}`
                                                ).join('\n\n');
